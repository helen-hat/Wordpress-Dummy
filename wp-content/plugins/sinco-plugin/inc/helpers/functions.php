...
{ 
	...
	$subject = file_get_contents(get_template_directory() . '/assets/fonts/font-awesome/css/all.min.css');
...
function sinco_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('sinco_redux_custom_fonts_load', 'sinco_redux_custom_fonts_load');
function sinco_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = sinco_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/sinco/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [sinco_social_share_output description]
 *
 ...
 */
function sinco_social_share_output( $icon, $color = false ) {
...
function sinco_get_social_icons()
{
	...
	if( sinco_set( $options, 'social_media' ) && is_array( sinco_set( $options, 'social_media' ) ) )
	{
		$total = count( sinco_set( $options, 'social_media' ) ) - 2;
		foreach( sinco_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = sinco_set( $social_icon, 'title');
			$class = sinco_set( $social_icon, 'class');
			$link = sinco_set( $social_icon, 'social_link');
			$icon = sinco_set( $social_icon, 'social_icon');
...
add_action('sinco_social_share_output_two', 'sinco_social_share_output_two');
...
<ul class="d-flex share-icon align-items-center style-none pt-10">
	<li><?php esc_html_e('Share:', 'sinco'); ?></li>
        
	<?php if($options->get( 'facebook_sharing' )):?>
    <li><a href="http://www.facebook.com/sharer.php?u=<?php echo esc_url(get_permalink($PostID)); ?>" target="_blank"><i class="bi bi-facebook"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'twitter_sharing' )):?>
    <li><a href="https://twitter.com/share?url=<?php echo esc_url(get_permalink($PostID)); ?>&text=<?php echo esc_attr($post_slug=$PostName); ?>" target="_blank"><i class="bi bi-twitter"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'linkedin_sharing' )):?>
    <li><a href="http://www.linkedin.com/shareArticle?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="bi bi-linkedin"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'pinterest_sharing' )):?>
    <li><a href="https://pinterest.com/pin/create/bookmarklet/?url=<?php echo esc_url(get_permalink($PostID)); ?>&description=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="bi bi-pinterest"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'reddit_sharing' )):?>
    <li><a href="http://reddit.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="bi bi-reddit"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'tumblr_sharing' )):?>
    <li><a href="http://www.tumblr.com/share/link?url=<?php echo esc_url(get_permalink($PostID)); ?>&name=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="bi bi-tumblr"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'digg_sharing' )):?>
    <li><a href="http://digg.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="bi bi-digg"></i></a></li>
    <?php endif;?>
</ul>
<?php }


function sinco_get_social_icon()
{
    ...
    <?php $icons = $options->get( 'icons_social_share' );
		...
		<ul class="d-flex social-icon style-none">
			...
			<li><a target="_blank" href="<?php echo esc_url(sinco_set( $header_social_icons, 'url' )); ?>" style="background-color:<?php echo esc_attr(sinco_set( $header_social_icons, 'background' )); ?>; color: <?php echo esc_attr(sinco_set( $header_social_icons, 'color' )); ?>"><i class="fab <?php echo esc_attr( sinco_set( $header_social_icons, 'icon' ) ); ?>"></i></a></li>
            <?php endforeach; ?>
		</ul>
	<?php endif; ?>
<?php }
