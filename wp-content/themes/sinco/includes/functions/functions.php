...
function sinco_the_breadcrumb() {
	...
	if ( ! is_front_page() ) {
		...
		/** If on a single job post */
		if ( is_singular('job_posting') ) {
			// Manually define the Career page URL
			$career_page_url = 'http://careers.local/careers-new/';
			
			// Add the "Careers" link 
			$breadcrumb .= $before . '<a href="' . esc_url($career_page_url) . '">' . esc_html__('Careers', 'sinco') . '</a>' . $after;
			
			// Add the current job post title
...
function sinco_comment_form( $args = array(), $post_id = null ) {
	...
	$fields              = array(
		...
		<input id="author" name="author"  placeholder="' . esc_attr__( 'Name', 'sinco' ) . '" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" maxlength="245"' . $aria_req . $html_req . ' />
		</div>',
		'email'  => '<div class="input-wrapper mb-15 col-md-6 col-sm-6 col-xs-12">
		<input id="email" placeholder="' . esc_attr__( 'Email', 'sinco' ) . '" class="form-control" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" maxlength="100"/></div>',
